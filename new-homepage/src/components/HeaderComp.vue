  ...
  <header class="bg-teal-600 text-white shadow-md">
    <nav class="container mx-auto px-4">
      <div class="flex items-center justify-between h-16">
        <RouterLink to="/" class="text-xl font-bold hover:text-teal-200 transition-colors">
          Survivalな理想郷
        </RouterLink>
      ...
      <div v-if="isMobileMenuOpen" class="md:hidden pb-4">
        <ul class="space-y-2">
          <li v-for="menu in menuList" :key="menu.title">
            <RouterLink v-if="menu.isRouterLink" :to="menu.href" @click="closeMobileMenu" :class="[
              'block px-3 py-2 rounded hover:bg-teal-700 transition-colors',
              activeTitle === menu.title ? 'bg-teal-800 font-semibold' : ''
            ]">
              {{ menu.title }}
            </RouterLink>
            <a v-else :href="menu.href" :target="menu.target" @click="closeMobileMenu" :class="[
              'block px-3 py-2 rounded hover:bg-teal-700 transition-colors',
              activeTitle === menu.title ? 'bg-teal-800 font-semibold' : ''
            ]">
              {{ menu.title }}
