window.addEventListener('DOMContentLoaded', () => {
    for (const data of dataList) {
        const tblDOM = document.getElementById(data.id);
        if (!tblDOM) continue;
        let tblHTML = '<table><thead><tr><th>メーカー</th><th>パーツ名称</th></tr></thead><tbody>';
        for (const item of data.value) {
            tblHTML += `<tr><td>${item.maker}</td><td>${item.name}</td></tr>`;
        }
        tblHTML += '</tbody></table>';
...
const dataList = [
    {
        ...
        value: [{ maker: 'VZ Grips', name: 'Diamond Back MARSOC G10' }],
    },
    {
        id: 'WAM4Prime',
        value: [
    ...
    {
        id: 'VFCUMAREXVP9',
        value: [{ maker: 'CRUSADER', name: 'VP9 Steel Up-Grade Parts Kit' }],
    },
];
